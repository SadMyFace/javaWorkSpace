...
public class Method연습 {
	...
	public static void main(String[] args) {
		 ...
		 * 		- 정수형 : int, bye, short, ling(접미사 l(L)
		 * 		- 실수형 : double, float(접미사 f(F))
		 * 		- 정수형 : char(정수형, 아스키코드 문자표에 저장되어 있는 코드로 저장) : 1글자
		 * 		- 불라인 : boolean(true, false)
		 * 
		 ...
		 *	-증감연산자(++, --)
		 *		- 1씩 증가, 1씩 감소
		 *		- 위치에 따라 증가가 먼저되거나, 출력이 먼저 되거나 순서가 결정됨
		 * 		- i++, ++i
		 * 
		 * -조건선택연산자(조건식 ? true : false)
		 * 		- 조건식이 참이면 true, 거짓이면 false를 선택
		 * 		- 간편한 형태의 if 역할
		 * 
		 ...
		 * 		while문에서는 증감식이 실행문 안에 포함되기 때문에 조건 안에서 증감식을 찾지
		 ...
		 * 	- 배열의 총길이는 length로 확인 가능 -> 배열.length
		 ...
		 *  - static이 있으면 객체생성을 하지 않고, 클래스메서드, 클래스변수를 호출 가능
		 ...
		 * 
		 * 	- 멤버변수 사용
		 * 	객체명.멤버변수() => 일반적으로 멤버변수는 private로 선언되기 때문에 보통은 호출 불가능
		 * 
